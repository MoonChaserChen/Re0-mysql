...
1. 下载
    ...
    从https://dev.mysql.com/downloads/mysql/下载rpm包，可直接在连网服务器上使用wget命令下载。

2. 安装

    假设上面下载的文件为：mysql-community-server-5.7.20-1.el6.x86_64.rpm，执行以下命令安装：
    ```
...
3. 测试
    ...
    使用`netstat -nat | grep 3306`检查是否mysql成功安装并启动（默认mysql会监听3306端口）。
...
1. 配置repo
    ...
    `vim /etc/yum.repos.d/mysql-community.repo`写入以下内容：
    ```
...
3. 签名
    ...
    `vim /etc/pki/rpm-gpg/RPM-GPG-KEY-mysql` 写入以下内容：
...
2. 修改密码
    ...
    ```
    mysql> ALTER USER 'root'@'localhost' IDENTIFIED BY 'new_pass';
    mysql> grant all privileges on *.* to root@'%' identified by "new_pass";
    ```